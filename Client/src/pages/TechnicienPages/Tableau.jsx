...
function isMaintenanceFocused(item) {
...
export default function Tableau() {
    ...
    const activeEquipment = useMemo(() => equipment.filter((item) => !item.isDeleted), [equipment]);

    const maintenanceFocusList = useMemo(
        () => activeEquipment.filter((item) => isMaintenanceFocused(item)),
        [activeEquipment]
    );

    const stats = useMemo(() => {
        const maintenanceCount = activeEquipment.filter((item) => item.status === 'Maintenance').length;
        const brokenLikeCount = activeEquipment.filter(
            (item) => item.condition === 'Poor' || item.condition === 'Under Maintenance'
        ).length;

        return {
            total: activeEquipment.length,
            maintenance: maintenanceCount,
            brokenLike: brokenLikeCount,
            available: activeEquipment.filter((item) => item.status === 'Available').length
        };
    }, [activeEquipment]);
