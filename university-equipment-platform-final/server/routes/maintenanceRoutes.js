...
router.get('/technician-dashboard', authMiddleware, async (req, res) => {
  try {
    if (!canAccessMaintenance(req.user.role)) {
      return res.status(403).json({ success: false, message: 'Acces refuse.' });
    }

    const equipments = await Equipment.find({ isDeleted: { $ne: true } });
    const openTickets = await MaintenanceTicket.find({ status: 'open' }).populate('equipmentId');

    let totalCopies = 0;
    let copiesNeedingMaintenance = 0;
    let availableCopies = 0;
    let criticalCopies = 0;
    
    // Map variables to collect items to review
    const maintenanceFocusMap = new Map();

    equipments.forEach(eq => {
      totalCopies += (eq.quantity || 0);
      if (eq.status === 'Available') {
          availableCopies += (eq.quantity || 0);
      }
      
      if (eq.condition === 'Poor' || eq.condition === 'Under Maintenance' || eq.status === 'Maintenance') {
          if ((eq.quantity || 0) > 0) {
              criticalCopies += (eq.quantity || 0);
              maintenanceFocusMap.set(eq._id.toString(), {
                  _id: eq._id,
                  name: eq.name,
    ...
    openTickets.forEach(ticket => {
      copiesNeedingMaintenance += (ticket.quantity || 0);
      totalCopies += (ticket.quantity || 0);
      
      const eq = ticket.equipmentId;
      if (eq) {
          const eqId = eq._id.toString();
          if (maintenanceFocusMap.has(eqId)) {
              const current = maintenanceFocusMap.get(eqId);
              // Avoid incrementing tracking if we only want to list it once, 
              // but here we specifically want to track the total quantity needing review.
              current.quantity += ticket.quantity;
              current.status = 'En maintenance';
          } else {
              maintenanceFocusMap.set(eqId, {
                  _id: eq._id,
                  name: eq.name,
                  category: eq.category,
                  condition: eq.condition,
    ...
    return res.status(200).json({
      ...
      data: {
        stats: {
          total: totalCopies,
          maintenance: copiesNeedingMaintenance,
          brokenLike: criticalCopies,
          available: availableCopies
        },
        maintenanceFocusList: Array.from(maintenanceFocusMap.values())
      }
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: 'Erreur lors de la recuperation des statistiques dashboard',
      error: error.message
    });
  }
});
